...
router.post('/bankandotheroffers_submit', upload.single('fileNames'), function (req, res, next) {
    console.log(req.body)
    console.log(req.file)
    try {
        pool.query("insert into bankandotheroffers( status, filenames, created_at, updated_at, user_admin)values(?,?,?,?,?)",
            [ req.body.status,req.file.filename,req.body.created_at,req.body.updated_at,req.body.user_admin], function (error, result) {
                ...
                if (error) {
                    ...
                    res.status(200).json({ message: 'Bank Offer Submitted Successfully', status: true })
                }
